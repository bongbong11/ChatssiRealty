// prompts.js — 챗씨부동산
// 모든 프롬프트는 fullContext(최종 조립 프롬프트 전체, generateQuietPrompt류로 획득)를 입력으로 받음.
// 정보블록(다른 확장의 상태창 등) 무시 지시는 공통 가드 문구로 모든 프롬프트에 포함.

export const INFO_BLOCK_GUARD = `
⚠ 입력 컨텍스트 안에 다른 확장이 주입한 상태창/정보블록(고정 태그나 구분자로 감싸진 텍스트,
예: [STATUS], <status> 등)이 포함되어 있을 수 있다. 해당 블록은 참고하지 말고 완전히 무시할 것.
그 블록의 형식, 수치, 표현을 출력에 절대 반영하지 말 것.
`.trim();

export function buildWorldClassifyPrompt(fullContext, userHint) {
  return `
역할: 캐릭터 데이터 분석가.
${INFO_BLOCK_GUARD}
...
export function buildAddressGeneratePrompt(fullContext, worldClass, userHint) {
  return `
역할: 부동산 정보 생성기.
${INFO_BLOCK_GUARD}
...
작업: 캐릭터의 거주지 정보를 아래 항목으로 생성하라. 항목별 단답 위주, 줄글 금지(집 이야기 항목만 예외).
...
export function buildHouseMovePrompt(fullContext, worldClass, prevCard) {
  return `
...
작업: 위 컨텍스트를 바탕으로 새 거주지 카드를 생성하라.
같은 캐릭터의 톤/일관성은 유지하되, 기존 데이터를 그대로 복사하지 말 것.
출력 항목 구조는 buildAddressGeneratePrompt와 동일.

출력 형식: JSON만 출력 (구조 동일).
`.trim();
}

export function buildItemPoolPrompt(fullContext, worldClass, spaceKey, spaceLabel) {
  return `
역할: 소지품 인벤토리 생성기.
${INFO_BLOCK_GUARD}
...
2. 존재한다면, 입력 컨텍스트에서 실제로 확인되는 아이템과 캐릭터 성격/재산수준 기반
...
export function buildFoodListPrompt(fullContext, worldClass, subtype) {
