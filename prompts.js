...
⚠ HARD CONSTRAINT: every single item, including the special surprising ones, MUST be an
actual food, beverage, ingredient, or condiment that genuinely belongs in a ${subtype === "fridge" ? "fridge" : "pantry"} —
never a non-food object (no documents, maps, tools, weapons, keepsakes, electronics, etc.).
The "surprise" must come entirely from the BACKSTORY (why a food item is there, who left it,
what it represents) — never from the item itself being a non-food/out-of-place object. If you
can't think of a surprising backstory for an actual food item, just make it a fitting but
unremarkable food item instead — do not break the food-only rule to force a "twist."
